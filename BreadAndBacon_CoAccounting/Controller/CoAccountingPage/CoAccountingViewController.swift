...
class CoAccountingViewController: UIViewController {
    ...
    @IBAction func addDetail(_ sender: UIButton) {
        guard let presentCoDetailVC = self.storyboard?.instantiateViewController(withIdentifier: ID.addCoDetailVCID) as? AddCoDetailViewController
        else {
    ...
    @objc func refresh(sender: UIRefreshControl) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            self.fetchCoBookDetail(document: self.didSelecetedBook, subCollection: self.subCategory.coExpenditure)
...
extension CoAccountingViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let presentEditAddCoVC = self.storyboard?.instantiateViewController(withIdentifier: ID.addCoDetailVCID) as? AddCoDetailViewController
        else {
            fatalError(errorMessage.fatalErrorMSGCoDetailVC)
        }
    ...
    func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { suggestedActions -> UIMenu? in
            let deleteAction = UIAction(title: "刪除", image: nil, identifier: nil, discoverabilityTitle: nil, attributes: .init(), state: .off) { action in
                BBCoFireBaseManager.shared.deleteSpecificData(accountData: self.data, document: self.didSelecetedBook, subCollection: self.subCategory.coExpenditure, indexPathRow: indexPath.row)
                self.fetchCoBookDetail(document: self.didSelecetedBook, subCollection: self.subCategory.coExpenditure)
            }
...
extension CoAccountingViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let listCell = tableView.dequeueReusableCell(withIdentifier: ID.listCellID) as? ListTableViewCell
        else {
